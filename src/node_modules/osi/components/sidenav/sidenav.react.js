...
class SideNavigation extends Component {
  ...
  render () {
    const disabled = true
    const menuItems = [
            {title: 'dashboard', icon: 'tachometer', route: '/dashboard', style: {paddingTop: '6px'}},
            {title: 'Portfolio', icon: 'newspaper-o', route: '/portfolios', style: {paddingTop: '10px'}},
    ...
    const adminItems = [
            { title: 'admin', icon: 'database', route: '/admin' },
    ...
    return (
      <div className='sidenav'>
        <ul>
          {menuItems.map((item, index) => <SidenavItem key={index} item={item} />)}
        </ul>
        <div className='spacer' />
        {this.props.isAdmin && (
          <div className='admin'>
            <ul>
              {adminItems.map((item, index) => <SidenavItem key={index} item={item} />)}
            </ul>
          </div>
        )}
      </div>
