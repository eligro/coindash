...
import { ERC20Data } from './ETHTransaction'
...
export class ETHChainAccount extends Account {
  ...
  fetchAllBalances (tokens, tokenIdx, callback) {
    let cntActions = 0
    tokens && tokens.length && tokens.map((token, i) => {
      cntActions++

      return this.fetchBalanceForToken(token, function (t, balance) {
        t.balance = balance
  ...
  fetchAllTokenContractTxList (idx, tokens, balances, account, callback) {
    ...
    tokens && tokens.length && tokens.map((token, i) => {
      ...
      return this.fetchTokenContractTxList(t, account, function (txs) {
        balances.push(txs)
