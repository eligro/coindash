...
class Card extends Component {
  render () {
    ...
    return (
      <div {...props} className={className} key={title}>
        {((title || actions) &&
        <card-header key='card-head'>
          {(title && <h3>{title}</h3>) || <span className='spacer' />}
          {actions && <div className='actions'>{actions}</div>}
        </card-header>
          )}
        <card-content key='card-content'>{children}</card-content>
        <footer key='abc'>{footer}</footer>
      </div>
    )
  }
}
