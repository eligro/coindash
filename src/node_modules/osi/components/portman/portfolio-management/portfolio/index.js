...
class Portfolio extends Component {
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (nextProps.calculationError) {
      console.log('see calc error:', nextProps.calculationError)
      if (!this.state.alertOpen || this.state.alertId !== 'calcError') {
        this.setState({
          alertOpen: true,
          alertTitle: 'Calculation error',
          alertText: 'Theres an error in your calculations',
          alertId: 'calcError'
        })
      }
    }
  ...
  checkIfReady () {
    const { calculations, portfolio, portfolioActions, process: pProcess, balances } = this.props
    const hasPortoflio = !!portfolio
    const hasCalculation = portfolio && !!portfolio.lastCalculated
    const hasProcess = hasCalculation && pProcess
    const hasBalances = portfolio && !!portfolio.lastBalances
    const isFetching = (hasProcess && pProcess.fetching) || (hasBalances && balances.fetching)
    const calculationHere = (hasCalculation && !!calculations) || (hasPortoflio && !hasCalculation)

    if (!isFetching && hasPortoflio && calculationHere) {
  ...
  render () {
    ...
    const onboarding = addressList.length === 0
        ...
        <PageContent>
          <header className='tested'>
            ...
            <div className='onboarding'>
              <MessageBlock className='onboarding' actions={[
                <Button
                  key='associate-addresses'
                  raised
                  primary
                  label='Associated address with portfolio'
                  onClick={e => this.openAssociateAddress()} />
              ]}>
                <h3>Associate Addresses</h3>
                <p>
                      To begin using Coindash powerful analysis features you need to
                      associate on or more Ethereum addresses with your portfolio.
                    </p>
                  ...
                  <CalculatePortfolio
                    ...
                    button={(
                      <Button
                        raised
                        primary
                        label='Portfolio calculates...' />
                        )}
                    statusElm={(
                      <div className='progress-status'>
                        <span className='msg'>
                          {processProgress.toLocaleString()}% Calculated
                            </span>
                        <CircularProgress
                          max={100}
                          min={0}
            ...
            <Card className='tokens'>
              ...
              <CardHeader
                ...
                <div className='actions'>
                  ...
                  <AddTokenDialog open={this.state.showAddToken} then={(...args) => this.setState({showAddToken: false})} />
                </div>
              </CardHeader>
          ...
          <footer>
            ...
            {this.props.singularPortfolio && (<NewPortfolio />)}
          </footer>
        </PageContent>
        ...
        <Toolbar className='one' style={{
          ...
          <ToolbarGroup style={alignToStart} className='actions' lastChild>
            ...
            <CalculatePortfolio
              ...
              processProgress={processProgress}
              button={(
                <IconButton tooltip='Reload Calculations'>
            ...
            <IconButton tooltip='Delete Portfolio'>
