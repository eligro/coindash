...
class Portfolio extends Component {
  ...
  onAssociateAddress (address) {
    ...
    console.log('adding address to portfolio:', address, portfolio)
    portfolioActions.associateAddressToPortfolio(portfolio, address)
      .then(_ => {
        console.log('address added:', address, portfolio)
        return true
      })
      .then(_ => this.setState({associateOpen: false}))
      .then(_ => {
        console.info('reload user portfolios')
        portfolioActions.loadUserPortfolios(this.props.user.profile.uid)
      })
  }

  startPortfolio () {
    const { portfolio, chartActions } = this.props
    console.log('starting!', portfolio)
    const { addresses = {} } = portfolio.portfolio
    const addsArray = Object.keys(addresses).map(k => addresses[k])
    console.log('addresses:', addsArray)
    if (!addsArray || !addsArray.length) {
      this.setState({
        alertOpen: true,
    ...
    // chartActions.calcPortfolio(portfolio)
  }
  render () {
...
class AssociateDialog extends PureComponent {
  ...
  addressUpdate () {
    ...
    const { addresses = {} } = this.props.portfolio.portfolio
    const addsArray = Object.keys(addresses).map(k => addresses[k])
    const addressExists = addsArray.indexOf(address) >= 0
    ...
    if (
      ...
      this.setState({isAddress, addressExists, error: isAddress && !addressExists ? false : error})
    }
  }
...
class AlertDialog extends Component {
  render () {
    console.log('alert dialog this:', this)
    console.log('alert dialog porps:', this.porps)
