...
class PositionItem extends React.Component {
  ...
  render () {
    ...
    return (
      <Card title={`${item.short} — ${item.long}`} className={`position  ${isIncrease ? `increase` : `decrease`}`} headerActions={actions}>
        <h3>{tls(this.props.item.price)} <small>{nf(this.props.item.cap24hrChange)}%</small></h3>
        <card-footer>
          <p><value>{comm((+this.props.item.mktcap).toFixed(2))}</value> <label>Mktcap</label></p>
          <p><value>{comm(this.props.item.volume)}</value> <label>Volume</label></p>
        </card-footer>
      </Card>
    )
  }
