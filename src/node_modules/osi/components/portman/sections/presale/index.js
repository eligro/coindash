    ...
    return (
      <div className='presale'>
        ...
        <PageContent>
          ...
          <multi>
            {!this.state.hasCoupon && <EnterCoupon
              attemptCoupon={e => this.attemptCoupon(e)}
              onChange={(e, v) => this.onCouponValueChanged(e, v)}
              error={this.state.insertCouponError} />}
            {this.state.hasCoupon &&
            <MessageBlock className='insert-coupon' actions={(
              <RaisedButton onClick={e => this.setState({revealCode: !this.state.revealCode})} primary label='Reveal ICO Address' />
                )}>
...
const EnterCoupon = ({attemptCoupon, error, onChange, ...props}) => <MessageBlock className='insert-coupon' actions={(
  <RaisedButton onClick={attemptCoupon} type='submit' label='Attempt Coupon' />
)}>
  <h3>Have a coupon?</h3>
  <form className='coupons' onSubmit={attemptCoupon}>
    <TextField
