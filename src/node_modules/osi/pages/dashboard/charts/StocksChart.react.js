...
const tls = i => i.toLocaleString(...currencyFormat)
const nf = i => new Intl.NumberFormat().format(i)
    ...
    const config = {
      ...
      tooltip: {
        formatter: function () {
          ...
          console.log('balanaces?', balances)
          let balancesStr = '<br/>'
          let bals = balances.sort((a, b) => a.fiatValue < b.fiatValue).map(({symbol, balance, fiatValue, fiatCurrency}) => `<code>\t\t ${symbol} \t</code>${nf(balance)}<code>\t</code>${tls(fiatValue)}`).join('<br/>')
          ...
          let ret = '<b>Portfolio Value ' + tls(dayDataByDate[portfolioValue.x].dayFiatValue) + ' (' + date.toLocaleDateString() + ')</b>'
