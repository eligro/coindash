...
class Users extends Component {
  ...
  loadUsers () {
    ...
    User.getUsers({
      fromIndex: (this.state.page - 1) * this.state.itemsPerPage,
      ...
      .then(users => {
        ...
        if (this.props.gmode) {
          ...
          this.setState({lastUserKey: cusers.pop().uid, firstUserKey: cusers.shift().uid})
          Promise.all(proms)
            .then(res => {
              this.setState({
                users: res
        ...
        {this.state.totalUsers > this.state.itemsPerPage && (
          <div className='page'>
            <RaisedButton label='Prev' disabled={!this.state.page === 1}
              onClick={e => {
                this.setState({page: this.state.page - 1, userKeyContext: this.state.firstUserKey})
                this.loadUsers()
              }} />
              {this.state.page}
            <RaisedButton label='Next' disabled={!this.state.page >= Math.ceil(this.state.totalUsers / this.state.itemsPerPage)}
              onClick={e => {
