...
export const isAdmin = uid => {
  ...
  let adminRef = db.ref(`admin/${usersPath}/${uid}`)
...
export const getUsers = ({fromIndex = 0, limit = 50} = {}) => {
  let usersRef = db.ref(usersPath)
  console.log('what are getUsers', fromIndex, limit)
  return new Promise((resolve, reject) => {
    let children = []
    usersRef = usersRef

      .startAt(fromIndex)
      .limitToFirst(limit).once('value', snp => {
        snp.forEach(csnp => {
...
export const makeAdmin = (user) => {
  let adminUserRef = db.ref(`admin/${usersPath}/${user.uid}`)
...
export const unmakeAdmin = (uid) => {
  let adminUserRef = db.ref(`admin/${usersPath}/${uid}`)
  return adminUserRef.remove()
}

export const enableUser = (uid) => {
  let ref = db.ref(`${usersPath}/${uid}`).child('disabled')
