      ...
      tooltip: {
        formatter: function () {
          ...
          let balances = _balances
            ...
            .map(({symbol, balance, fiatValue, fiatCurrency}) => `<tr><td><span>${symbol}<span>\t\t</span></span></td><td>${nf(balance)}<span>\t\t</span></td><td>${tls(fiatValue)}</td></tr>`).join('')
